...
SMILECOMPONENT_REGCOMP(cMZcr)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("zcr","(1/0=yes/no) compute zero-crossing rate",1);
    ct->setField("mcr","(1/0=yes/no) compute mean crossing rate",1);
    ct->setField("amax","(1/0=yes/no) compute maximum absolute sample value",1);
    ct->setField("maxmin","(1/0=yes/no) compute maximum and minimum sample value",1);
  )
...
/*
int cMZcr::configureWriter(const sDmLevelConfig *c)
{
...
int cMZcr::processVectorInt(const INT_DMEM *src, INT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
