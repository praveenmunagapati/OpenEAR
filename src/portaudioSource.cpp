...
SMILECOMPONENT_REGCOMP(cPortaudioSource)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("audioBuffersize","size of port audio recording buffer in samples (overwrites audioBuffersize_sec, if set)",0);
...
//    printf("no wait! %i  %i bs=%i\n",writer->getNAvail(), (writer->checkWrite(audioBuffersize)), audioBuffersize);
  }
...
int pcmDataToMatrix(const void *buf, cMatrix *m, int nBPS, int nBits, int nChan, int monoMixdown)
{
    int i,c;
                           ...
                           void *userData )
{
  ...
  if (writer->checkWrite(framesPerBuffer)) {
    ...
    pcmDataToMatrix(inputBuffer, mat, obj->getNBPS(), obj->getNBits(), obj->getChannels(), obj->isMonoMixdown());
