...
SMILECOMPONENT_REGCOMP(cVectorProcessor)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN( {}
    ct->setField("processArrayFields","1 = process each array field as one vector individually (and produce one output for each input array field), and process only array fields / 0 = process complete input frame as one vector",1);
  )
...
int cVectorProcessor::dataProcessorCustomFinalise()
{
  ...
  if (!processArrayFields) {
    ...
    if (!namesAreSet) {
      fNo[0] = 0; 
      for (i=0; i<Nfi; i++) {  // TODO::: this is NOT OK... why do we run this for each input field?? (names...?)
        int arrNameOffset=0;
...
//new:
    ...
    for (i=0; i<Nfi; i++) {
      ...
      if (__N > 1) {
        ...
        SMILE_WRN(1,"ignoring single (non-array) field '%s'!",tmp);
      }
    }
