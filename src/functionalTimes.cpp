...
#define FUNCT_UPLEVELTIME        12
#define FUNCT_DOWNLEVELTIME      13


#define N_FUNCTS  14

#define NAMES     "upleveltime25","downleveltime25","upleveltime50","downleveltime50","upleveltime75","downleveltime75","upleveltime90","downleveltime90","risetime","falltime","leftctime","rightctime","upleveltime","downleveltime"
#define IDX_VAR_FUNCTS 12  // start of uptime, downtime, arrays of variable length
...
SMILECOMPONENT_REGCOMP(cFunctionalTimes)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("norm","how to normalise times: turn (norm to range 0..1 using turnlength) / second (abs. times in seconds) / frame (abs. number of frames)","turn");
  )
...
void cFunctionalTimes::fetchConfig()
{
  ...
  if (Norm != NULL) {
    if (!strcmp(Norm,"turn")) norm=NORM_TURN;
    else if (!strcmp(Norm,"second")) norm=NORM_SECOND;
    else if (!strcmp(Norm,"frame")) norm=NORM_FRAME;
  }
  ...
  if (getInt("upleveltime75")) enab[FUNCT_UPLEVELTIME50] = 1;
  if (getInt("downleveltime75")) enab[FUNCT_DOWNLEVELTIME50] = 1;
