...
SMILECOMPONENT_REGCOMP(cCsvSink)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN_BEGIN
    ...
    ct->setField("delimChar","delimiter character (usually ',' or ';')",';');
    ...
    ct->setField("printHeader","print header with attribute names (1/0 = yes/no)",1);
...
void cCsvSink::fetchConfig()
{
  ...
  if (append) SMILE_DBG(3,"printing header with feature names");

  number = getInt("number");
  if (append) SMILE_DBG(3,"printing of frameIndex (number) to file is enabled");
...
/*
  ...
  */
  ...
  fprintf(filehandle,"%e%s",vec->dataF[0],NEWLINE);
