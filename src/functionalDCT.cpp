...
SMILECOMPONENT_REGCOMP(cFunctionalDCT)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ct->setField("firstCoeff","first DCT coefficient to compute",1);
    ct->setField("lastCoeff","last DCT coefficient to compute",6);
	ct->setField("nCoeffs","alternative to lastCoeff (only parsed if lastCoeff is not set): number DCT coefficient to compute",5);
  )
...
void cFunctionalDCT::fetchConfig()
{
  ...
  if (isSet("nCoeffs")) {
    lastCoeff = firstCoeff + getInt("nCoeffs");
