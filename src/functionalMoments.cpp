...
SMILECOMPONENT_REGCOMP(cFunctionalMoments)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ct->setField("variance","1/0=enable/disable computation of variance",1);
    ct->setField("stddev","1/0=enable/disable computation of standard deviation",1);
    ct->setField("skewness","1/0=enable/disable computation of skewness",1);
    ct->setField("kurtosis","1/0=enable/disable computation of kurtosis",1);
...
long cFunctionalMoments::process(FLOAT_DMEM *in, FLOAT_DMEM *inSorted, FLOAT_DMEM min, FLOAT_DMEM max, FLOAT_DMEM mean, FLOAT_DMEM *out, long Nin, long Nout)
{
  ...
  if ((Nin>0)&&(out!=NULL)) {
    ...
    if (enab[FUNCT_SKEWNESS]) {
      if (m2 > 0.0) out[n++]=(FLOAT_DMEM)( m3/(Nind*sqm2*sqm2*sqm2) );
      else out[n++] = 0.0;
    }
    if (enab[FUNCT_KURTOSIS]) {
      if (m2 > 0.0) out[n++]=(FLOAT_DMEM)( m4/(Nind*sqm2*sqm2*sqm2*sqm2) );
      else out[n++] = 0.0;
    }
