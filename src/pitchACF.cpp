...
SMILECOMPONENT_REGCOMP(cPitchACF)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN( {} // <- this is only to avoid compiler warnings...
    ...
    ct->setField("maxPitch","Maximum detectable pitch", 500.0);  
    ct->setField("voiceProb","1/0 = on/off voiceProb",1);
	ct->setField("voiceQual","1/0 = on/off voice 'quality' (zcr of acf)",0);
    ct->setField("HNR","1/0 = on/off HNR",1);
    ct->setField("F0","1/0 = on/off F0 (pitch)",1);
    ct->setField("F0env","1/0 = on/off F0 envelope (expontial decay smoothing)",0);
	ct->setField("voicingCutoff","voicing Prob. threshold for pitch detection [0.0 - 1.0]",0.55);
...
/*
int cPitchACF::myConfigureInstance()
{
  int ret=1;
...
/*
int cPitchACF::configureWriter(const sDmLevelConfig *c)
{
...
/*void cPitchACF::configureField(int idxi, long __N)
{
//     const sDmLevelConfig *c = reader->getLevelConfig();
// idxc = getFconf(idxi);
}
...
/*
int cPitchACF::myFinaliseInstance()
{
  int ret=1;
...
/*int cPitchACF::processVectorInt(const INT_DMEM *src, INT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
...
int cPitchACF::processVectorFloat(const FLOAT_DMEM *src, FLOAT_DMEM *dst, long Nsrc, long Ndst, int idxi) // idxi=input field index
{
  ...
  if (F0 || F0env || voiceQual) {
    ...
    if (voicing < voicingCutoff) { maxIdx = 0; }  //vq/maxPitch
    ...
    FLOAT_DMEM tol = 0.4;
    FLOAT_DMEM alpha = 0.3;

    if (pitch > 0.0) {
      if (glMeanPitch == 0.0) glMeanPitch = pitch;
      //if ((lastPitch == 0.0)||(onsFlag==1)) tol = 0.1;
      if (!((pitch < (1.0+tol)*glMeanPitch)&&(pitch > (1.0-tol) * glMeanPitch))) {
        pitch = glMeanPitch;
        alpha /= 3.0;
      } 
      if (onsFlag&&(lastPitch > pitch)) lastPitch *= 0.85;
    } 
    if ((pitch>0.0)&&(onsFlag==-1)) { lastPitch = pitch; }


    if (oPitch > 0.0)
      glMeanPitch = (1.0-alpha)*glMeanPitch + alpha*oPitch;    // TODO: pitch decay constant!


    FLOAT_DMEM out;
    if ((lastlastPitch!=0.0)&&(lastPitch!=0.0))
      out = 0.5*(lastlastPitch + lastPitch);
    else 
      out = lastPitch;
    ...
    if (F0env) {
      if (out > 0.0)
        pitchEnv = 0.75*pitchEnv + 0.25*out;
      dst[n++] = pitchEnv;
    }
  }
