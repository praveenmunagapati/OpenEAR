...
SMILECOMPONENT_REGCOMP(cTonefilt)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("nNotes","number of semi-tone filters in bank",48);
    ct->setField("firstNote","frequency of first note in Hz",55.0);
    ct->setField("decayF0","gliding average decay coefficient for first note",0.9995);
    ct->setField("decayFN","gliding average decay coefficient for last note (must be < decayF0!)",0.998);
    ct->setField("outputPeriod","period of output frames in seconds",0.1);
