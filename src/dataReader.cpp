...
SMILECOMPONENT_REGCOMP(cDataReader)
{
  ...
  ct->makeMandatory(ct->setField("dmLevel", "dataMemory level to read from", (char *)NULL, ARRAY_TYPE));
  ...
  ct->setField("errorOnFullInputIncomplete", "1/0 = yes/no : 1 = abort with an error if full input matrix reading is activated (frameSize=0 & frameStep=0) and beginning of matrix (curR) is not 0, (if set to 0 only a warning is shown)", 1);
