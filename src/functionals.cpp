...
int cFunctionals::doProcess(int idxi, cMatrix *row, FLOAT_DMEM*y)
{
  ...
  if (requireSorted) {
    sorted = (FLOAT_DMEM*)malloc(sizeof(FLOAT_DMEM)*row->nT);
    // quicksort:
    memcpy( sorted, unsorted, sizeof(FLOAT_DMEM) * row->nT );
    // TODO: check for float_dmem::: with #if ...
    #if FLOAT_DMEM_NUM == FLOAT_DMEM_FLOAT
    smileUtil_quickSort_float(sorted, row->nT);
    #else
    smileUtil_quickSort_double(sorted, row->nT);
    #endif
  }
  ...
  for (i=0; i<nFunctionalsEnabled; i++) {
    if (functObj[i] != NULL) {
      ...
      int ret;
      ret = functObj[i]->process( unsorted, sorted, min, max, (FLOAT_DMEM)mean, curY, row->nT, functN[i] );
      if (ret < functN[i]) {
        int j;
