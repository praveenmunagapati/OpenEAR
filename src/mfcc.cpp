...
SMILECOMPONENT_REGCOMP(cMfcc)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("firstMfcc","first MFCC to compute",1);
    ct->setField("lastMfcc","last MFCC to compute",12);
    ct->setField("nMfcc","this allows to specify the  number of MFCC, instead of lastMfcc",12);
    ct->setField("melfloor","minimum value of melspectra when computing mfcc (will be forced to 1.0 when htkcompatible=1)",0.00000001); 
	// 
    ct->setField("cepLifter","parameter for cepstral 'liftering', set to 0.0 to disable cepstral liftering",22.0);
    ct->setField("htkcompatible","append 0-th coefficient at end instead of beginning (1/0 = yes/no)",1);
  )
