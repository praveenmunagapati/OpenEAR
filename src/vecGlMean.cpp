...
SMILECOMPONENT_REGCOMP(cVecGlMean)
{
  ...
  SMILECOMPONENT_IFNOTREGAGAIN(
    ...
    ct->setField("initFile","file to load initial mean values from",(const char*)NULL);
    ct->setField("updateMethod","how to incrementally update the mean values, one of the following:\n   'nul'  do not perform any updates, use only inital values\n   'exp'  exponential: m1 = alpha * m0  + (1-alpha) * x\n   'fix'  mean over buffer of fixed length\n   'avg'  moving average method with weighted fixed inital values\n   'iavg'  moving average method with weighted variable (updated per turn) inital values\n   NOTE: if 'resetOnTurn'==0 then 'avg' and 'iavg' are identical","iavg");
    ct->setField("alpha","weighting factor alpha in gliding average computation",0.995);
    ct->setField("weight","weighting factor for MAP mean update (julius lvcsr method)",100);
    ct->setField("fixedBuffer","size of buffer of fixed length [in seconds] (for fixed buffer mean, and for MAP method)",5.0);

    ct->setField("turnOnlyUpdate","perform mean update only during turns (works for all three methods)",1);
    ct->setField("invertTurn","invert turn state (i.e. turns a 'turnOnly' option into 'not turn' option)",0);
    ct->setField("resetOnTurn","reset mean values at the beginning of each new turn",0);

    ct->setField("turnOnlyNormalise","normalise only turns (in between data will pass through unmodified)",0);

    ct->setField("htkcompatible","flag that indicates wheter last coefficient in 'initFile' is loaded into means[0] (use this when reading htk-compatible cmn init files, and NOT using htk-compatible mfccs)",0);
...
void cVecGlMean::fetchConfig()
{
  ...
  SMILE_DBG(2,"initFile = '%s'",initFile);
  
  alpha = (FLOAT_DMEM)getDouble("alpha");
  SMILE_DBG(2,"alpha = %f",alpha);

  weight = (FLOAT_DMEM)getDouble("weight");
  SMILE_DBG(2,"weight = %f",weight);

  const char * updateMethodStr = getStr("updateMethod");
  SMILE_DBG(2,"updateMethod = '%s'",updateMethodStr);
  ...
  SMILE_DBG(2,"zeroThIsLast (htkcompatible) = %i",zeroThIsLast);

  skipNfirst = getInt("skipNfirst");
  SMILE_DBG(2,"skipNfirst = %i",skipNfirst);
  
  turnOnlyUpdate = getInt("turnOnlyUpdate");
  SMILE_DBG(2,"turnOnlyUpdate = %i",turnOnlyUpdate);

  turnOnlyNormalise = getInt("turnOnlyNormalise");
  SMILE_DBG(2,"turnOnlyNormalise = %i",turnOnlyNormalise);

  resetOnTurn = getInt("resetOnTurn");
  SMILE_DBG(2,"resetOnTurn = %i",resetOnTurn);

  invertTurn = getInt("invertTurn");
  SMILE_DBG(2,"invertTurn = %i",invertTurn);

  fixedBuffer = getDouble("fixedBuffer");
  SMILE_DBG(2,"fixedBuffer = %f",fixedBuffer);
}
...
int cVecGlMean::processComponentMessage( cComponentMessage *_msg )
{
  ...
  if (isMessageType(_msg,"turnStart")) {
    ...
    SMILE_IDBG(3,"received turnStart");
    return 1;
  }
  return 0;
}
